...
@section('content')
    <form action="{{ route('faqs.update', $faq->id) }}" method="POST" class="needs-validation" onsubmit="updateEditorContent()">
        @csrf
        @method('PUT')
                ...
                <div class="page-header">
                    <div class="page-header-left d-flex align-items-center">
                        <div class="page-header-title">
                            <h5 class="m-b-10">Редактировать FAQ</h5>
                        </div>
                        <ul class="breadcrumb">
                                        ...
                                        @foreach (['uz', 'en', 'ru'] as $lang)
                                            <div class="tab-pane fade show {{ $lang == 'uz' ? 'active' : '' }}" id="{{ $lang }}Content">
                                                <div class="form-group pb-3">
                                                    ...
                                                    <input type="text" class="form-control" id="question_{{ $lang }}" name="question_{{ $lang }}" value="{{ old('question_' . $lang, $faq->{'question_' . $lang}) }}" required>
                                                </div>
                                                <div class="form-group pb-3">
                                                    <label for="answer_{{ $lang }}">Ответ ({{ strtoupper($lang) }}):</label>
                                                    <div id="editor_{{ $lang }}" style="height:200px;">{!! old('answer_' . $lang, $faq->{'answer_' . $lang}) !!}</div>
                                                    <input type="hidden" id="text_{{ $lang }}" name="answer_{{ $lang }}" value="{{ old('answer_' . $lang, $faq->{'answer_' . $lang}) }}">
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
